odoo.define('pos_retail.pos_chanel', function (require) {
    ...
    exports.sync_backend = Backbone.Model.extend({ // chanel 2: pos sync backend
        ...
        sync_with_backend: function (data) {
            this.pos.set('synch', {state: 'warning', pending: 1});
            this.pos.datas_sync = _.filter(this.pos.datas_sync, function(cache) {
                return cache['model'] != data['model'] && cache['id'] != data['id']
            });
            this.pos.datas_sync.push(data);
            this.pos.update_caches();
